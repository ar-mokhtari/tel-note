...
type Storage protocol.JobStorage

func (AllJob *Storage) FindJobByChar(inputChar string) (status bool, res []uint) {
...
func (AllJob *Storage) FindJobByID(inputID uint) (bool, protocol.Job) {
	for _, data := range AllJob.JobData {
		if data.Id == inputID {
			return true, *data
		}
...
func (AllJob *Storage) NewJob(inputJob protocol.Job) bool {
	var LastID uint
	for _, data := range AllJob.JobData {
		if data.Id > LastID {
			LastID = data.Id
...
func (AllJob *Storage) EditJob(ID uint, newJob protocol.Job) bool {
...
func (AllJob *Storage) DeleteJob(IDS []uint) (resDel []uint) {
